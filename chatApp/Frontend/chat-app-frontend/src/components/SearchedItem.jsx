import React, { useState } from 'react'

import { useSelector } from 'react-redux';
import { MdDelete } from "react-icons/md";
import { useCheckpendingMutation } from '../utils/CheckPendingRequestSlice';
function SearchedItem({user,handlesend,handledelete,isfriend,ispending,handledeleteRequest}) {
    
   

  return (
    <div className="flex items-center justify-between p-2 bg-gray-700 rounded-lg shadow-sm" style={{"maxWidth":"500px"}}>
    
      <div className="flex items-center space-x-4">
        <div className="w-12 h-12 rounded-full bg-gray-600 overflow-hidden">
          {/* Placeholder for avatar */}
          <img
            src={user.profilepic}
            alt="Requester Name"
            className="w-full h-full object-cover"
          />
        </div>
        <span className="text-lg font-medium text-white">{user.username}</span>
      </div>
      <div className="flex space-x-2">

      {
  isfriend ? (
    <button
      className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition"
      onClick={() => handledelete(user._id)}
    >
      Remove
    </button>
  ) : ispending ? (
    <button
      className="px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600 transition" onClick={()=> handledeleteRequest(user) }
      
    >
      Delete Request
    </button>
  ) : (
    <button
      className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition"
      onClick={() => handlesend(user._id)}
    >
      Send Request
    </button>
  )
}
         
        
      </div>
      
     </div>
  )
}

export default SearchedItem